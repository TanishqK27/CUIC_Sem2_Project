...
def get_git_info() -> dict[str, str]:
    ...
    try:
        ...
        return {"branch": "unknown", "commits": "Error reading git", "status": "unknown"}
...
def get_team_status() -> list[dict[str, str]]:
    ...
    members = [
        "tan", "andrii", "ben", "dietrich", "alfie",
        "max", "miran", "mya", "isameel", "vansheeka", "james"
    ]
    roles = {
...
def generate_context() -> str:
    ...
    strategy_rows = "\n".join(
        f"| {s['name']} | {s['status']} | {s['lines']} |"
        for s in code["strategies"]
    )

    # Build client table
    client_rows = "\n".join(
        f"| {c['name']} | {c['status']} | {c['lines']} |"
        for c in code["clients"]
    )
