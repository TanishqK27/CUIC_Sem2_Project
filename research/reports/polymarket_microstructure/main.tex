...
% Font setup - Helvetica (sans-serif) to match NBA EDA report
\usepackage{helvet}
...
\begin{titlepage}
    ...
    \begin{tabular}{rl}
        \textbf{Data Coverage:} & January 26 -- February 12, 2026 \\[0.3cm]
...
This exploratory data analysis examines 18 days of Polymarket NBA prediction market activity (January 26 -- February 12, 2026), comprising 67.9 million orderbook events across 114 games and approximately 1.3 million executed trades. The collection period ends at the NBA All-Star break, which began February 13. The analysis investigates market microstructure characteristics for applications in quantitative trading and sports betting research, specifically characterizing liquidity conditions, price dynamics, and execution quality.
...
Price dynamics show extreme fat tails (kurtosis = 214, 95\% CI: [187, 249]), indicating that large price movements occur far more frequently than normal distribution assumptions suggest. Weak negative autocorrelation (half-life approximately 3 minutes) indicates mild mean reversion, though the magnitude may be insufficient to overcome transaction costs for most traders. Volatility follows a predictable game-phase lifecycle: rising through the first half, peaking in the third quarter, and collapsing as outcomes become certain.
...
\input{sections/02_data_overview}
\input{sections/03_orderbook}
\input{sections/04_price_dynamics}
\input{sections/05_trade_flow}
\input{sections/06_temporal}
\input{sections/07_cross_outcome}
\input{sections/08_nba_specific}
\input{sections/09_market_makers}
\input{sections/10_execution}
\input{sections/11_case_studies}
...
\input{methodology/tutorial_boxes}
\endgroup
